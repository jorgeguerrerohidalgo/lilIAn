...
def refresh_matter_alerts(
    ...
    count = update_overdue_status(matter_id, current_user.organization_id)

    return {"updated": count}


@router.get("/debug/document/{document_id}")
def debug_document_timeline(
    document_id: int,
    db: Session = Depends(get_db),
):
    """Debug endpoint to check if document has contract_timeline."""
    ...
    if not analysis:
        return {"error": "No analysis found", "document_id": document_id}

    import json
    timeline = analysis.contract_timeline

    return {
        "document_id": document_id,
        "has_timeline": timeline is not None,
        "timeline_is_list": isinstance(timeline, list),
        "timeline_length": len(timeline) if timeline else 0,
        "timeline_preview": timeline[:2] if timeline and isinstance(timeline, list) else None,
        "raw_timeline": str(timeline)[:500] if timeline else None,
    }


@router.post("/debug/generate-alerts/{document_id}")
def debug_generate_alerts(
    document_id: int,
    db: Session = Depends(get_db),
):
    """Debug endpoint to manually trigger alert generation."""
    ...
    try:
        ...
        logger.info(f"Debug: Generated {len(alert_ids)} alerts for document {document_id}")
        return {
            "success": True,
            "document_id": document_id,
        ...
        logger.error(f"Debug: Error generating alerts: {e}", exc_info=True)
        return {
            "success": False,
            "document_id": document_id,
            "error": str(e)
