  ...
  return (
    <div className="bg-cream rounded-xl border border-border p-6" aria-live="polite">
        ...
        <div className="mb-4">
          ...
          <div className="flex gap-3 flex-wrap">
            {validationSummary.errors > 0 && (
              <span className="px-3 py-1 text-sm font-medium rounded-full bg-coral-pale text-coral-dark">
                {validationSummary.errors} error{validationSummary.errors !== 1 ? "es" : ""}
              </span>
            )}
            {validationSummary.warnings > 0 && (
              <span className="px-3 py-1 text-sm font-medium rounded-full bg-amber-pale text-amber">
            ...
            {validationSummary.errors === 0 && validationSummary.warnings === 0 && validationSummary.required_missing.length === 0 && (
              <span className="px-3 py-1 text-sm font-medium rounded-full bg-green-pale text-green">
      ...
      {validationSummary && validationSummary.document_types_found && Object.keys(validationSummary.document_types_found).length > 0 && (
        <div className="mb-4">
          <h4 className="text-sm font-medium text-ink2 mb-2">Tipos de documentos detectados:</h4>
          <div className="flex gap-2 flex-wrap">
            {Object.entries(validationSummary.document_types_found).map(([docType, count]) => (
              <span
                key={docType}
                className="px-2 py-1 text-xs font-medium rounded bg-soft text-ink2"
              >
      ...
      {validationSummary && validationSummary.required_missing.length > 0 && (
        <div className="mb-4 p-3 bg-amber-pale border border-amber/20 rounded-lg">
          <h4 className="text-sm font-medium text-amber mb-2 flex items-center gap-2">
            <svg aria-hidden="true" className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            Documentos requeridos faltantes
          </h4>
          <ul className="text-sm text-amber space-y-1">
      ...
      {validationSummary && validationSummary.recommended_missing.length > 0 && (
        <div className="mb-4 p-3 bg-blue-pale border border-blue/20 rounded-lg">
          <h4 className="text-sm font-medium text-blue mb-2">Documentos recomendados faltantes</h4>
      ...
      {expanded && validationSummary && (
        <div className="mt-4 pt-4 border-t border-border space-y-4">
          {/* Documents list */}
          <div>
            <h4 className="text-sm font-medium text-ink2 mb-2">
              Documentos subidos ({documents.length})
            </h4>
            {documents.length === 0 ? (
              <p className="text-sm text-ink/60">No hay documentos</p>
            ) : (
              <ul className="space-y-2">
                {documents.map((doc) => (
                  <li key={doc.id} className="flex items-center justify-between text-sm">
                    <span className="text-ink">{doc.original_filename}</span>
